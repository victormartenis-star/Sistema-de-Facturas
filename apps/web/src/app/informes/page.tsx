...
function MargenBadge({ pct: p }: { pct: number }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-xs font-semibold tabular-nums ${cls}`}
    >
      {p.toFixed(1)}%
    </span>
  );
...
function MargenesChart({ rows }: { rows: ObrasKpiRow[] }) {
  ...
  return (
    <section className="mt-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <ResponsiveContainer
        width="100%"
        height={Math.max(160, data.length * 48 + 40)}
      >
        <BarChart
          layout="vertical"
          data={data}
      ...
      <div className="mt-2 flex items-center gap-4 text-xs text-gray-400">
        ...
        <span className="flex items-center gap-1">
          <span className="inline-block h-2 w-4 rounded-sm bg-yellow-200" />
          5–15%
        </span>
        <span className="flex items-center gap-1">
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map((r) => (
              <tr key={r.projectId} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[r.status] ?? 'bg-gray-100 text-gray-500'}`}
                  >
                ...
                <td className="px-4 py-3 text-right tabular-nums text-emerald-700 font-medium">
                  {fmt(r.totalCertificado)}
                </td>
                <td className="px-4 py-3 text-right tabular-nums text-gray-600">
                  {pct(r.pctCertificado)}
                </td>
                <td className="px-4 py-3 text-right tabular-nums text-gray-700">
                  {fmt(r.costReal)}
                </td>
                <td
                  className={`px-4 py-3 text-right tabular-nums font-medium ${r.margenBruto < 0 ? 'text-red-600' : 'text-emerald-700'}`}
                >
                  {fmt(r.margenBruto)}
                </td>
          ...
          <tfoot className="border-t-2 border-gray-300 bg-gray-50">
            <tr>
              ...
              <td
                className={`px-4 py-3 text-right font-semibold tabular-nums ${totMargen < 0 ? 'text-red-600' : 'text-emerald-700'}`}
              >
                {fmt(totMargen)}
              </td>
              <td className="px-4 py-3 text-right">
